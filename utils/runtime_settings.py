...
INFO_TEXT_KEY = "info_text"
DEFAULT_INFO_TEXT = "Информация о боте появится позже."
...
async def load_runtime_settings() -> None:
    ...
    info = await run(sheets.get_setting, INFO_TEXT_KEY)
    if info:
        _cache[INFO_TEXT_KEY] = info
...
def get_info_text() -> str:
    return _cache.get(INFO_TEXT_KEY, DEFAULT_INFO_TEXT)


async def set_info_text(text: str) -> None:
    sheets = get_sheets()
    await run(sheets.set_setting, INFO_TEXT_KEY, text)
    _cache[INFO_TEXT_KEY] = text
