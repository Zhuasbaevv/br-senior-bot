...
from config import (
    ...
    SENIOR_GROUPS, INTERNAL_API_SECRET, RANKS,
...
def _visible_targets(user: UserInfo) -> list[UserInfo]:
    """Зеркалит _visible_targets из handlers/admin_panel.py бота — один и тот же
    набор целей что в боте, что на сайте, для одной и той же роли."""
    ...
    return [u for u in all_users() if u.org == user.org] if user.org else []
...
async def admin_settings_pick(request: Request, user: UserInfo = Depends(get_current_user)):
    if user.role < Role.LEADERSHIP:
        return RedirectResponse("/profile")
    targets = [u for u in all_users() if u.telegram_id != user.telegram_id]
