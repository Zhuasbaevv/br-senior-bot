...
def _visible_targets(user: UserInfo) -> list[UserInfo]:
    """Кого user вправе видеть/администрировать — строго нижестоящих в своей
    организации/направлении (utils.access.can_manage). Раньше сюда попадали ВСЕ
    из своей организации/направления, включая равных и старших по роли."""
...
async def admin_user_view(telegram_id: int, request: Request, user: UserInfo = Depends(get_current_user)):
    """Зеркалит карточку статистики из бота (кнопка в 'Список пользователей' ->
    статистика/баллы/наказания) — раньше на сайте была только сама таблица без
    возможности что-либо сделать с конкретным человеком."""
    if user.role < Role.LEADER:
        return RedirectResponse("/profile")
...
async def admin_grant_role_submit(
    ...
    # КРИТИЧНО: role_value и org_or_group приходят из тела POST-запроса и легко
    # подделываются (curl/Postman) в обход выпадающих списков формы — раньше отсюда
    # можно было выдать себе/союзнику роль ВЫШЕ положенного (например, Старший
    # следящий отправлял role_value=6 и получал Создателя). Проверяем здесь и то,
    # и то, что цель вообще управляема этим админом (не равна/выше, в его зоне).
    grant_error = None
...
async def admin_review_approve(app_type: str, app_id: str, user: UserInfo = Depends(get_current_user)):
    ...
    # Раньше можно было одобрить/отклонить заявку чужой организации, просто подставив
    # её ID в URL — список заявок фильтровался по зоне видимости, а сами эти роуты
    # (approve/reject) — нет.
...
async def admin_logs_show(log_key: str, nick: str, request: Request, user: UserInfo = Depends(get_current_user)):
    if user.role < Role.SENIOR_WATCHER or log_key not in _LOG_SHEET_BY_KEY:
        return RedirectResponse("/admin/logs")
    # Прямой переход по URL /admin/logs/<key>/<чужой_ник> в обход списка выше —
    # перепроверяем зону видимости и здесь.
...
def _resolve_quiz_scope(user: UserInfo, scope_choice: str, scope_value: str) -> list[str] | None:
    """Превращает выбор из формы в конкретный список организаций, ЗАНОВО проверяя
    его на сервере (форма — это просто HTML, отправить POST можно и в обход неё)."""
    if user.role >= Role.LEADERSHIP:
        if scope_choice == "all":
            return ALL_ORGS
...
async def quiz_take_page(quiz_id: int, request: Request, user: UserInfo = Depends(get_current_user)):
    sheets = get_sheets()
    quiz = await _run(sheets.get_quiz, quiz_id)
    if not quiz or user.org not in sheets.quiz_orgs(quiz):
        return RedirectResponse("/profile")
