...
from utils.access import get_user, all_users, visible_orgs
...
async def cb_show_logs(callback: CallbackQuery):
    ...
    if not target or target.org not in visible_orgs(admin):
        await callback.answer("❌ Этот человек вне вашей зоны ответственности.", show_alert=True)
        return
