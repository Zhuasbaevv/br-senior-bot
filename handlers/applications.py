...
async def extra_photo(message: Message, state: FSMContext):
    if message.media_group_id:
        # Тот же приём, что и в отчётах (см. report_photo/_flush_album выше) — дебаунс
        # 1.5с, чтобы собрать ВСЕ фото альбома, а не только первое пришедшее сообщение.
        gid = message.media_group_id
        _extra_album_buffers.setdefault(gid, []).append(message)
        if gid in _extra_album_tasks:
            _extra_album_tasks[gid].cancel()
        _extra_album_tasks[gid] = asyncio.create_task(_flush_extra_album(gid, state))
        return

    await state.update_data(screenshots=[message.photo[-1].file_id])
...
_extra_album_buffers: dict[str, list[Message]] = {}
_extra_album_tasks: dict[str, asyncio.Task] = {}


async def _flush_extra_album(gid: str, state: FSMContext):
    try:
        await asyncio.sleep(_ALBUM_DEBOUNCE_SEC)
    except asyncio.CancelledError:
        return
    messages = _extra_album_buffers.pop(gid, [])
    _extra_album_tasks.pop(gid, None)
    if not messages:
        return
    file_ids = [m.photo[-1].file_id for m in messages]
    await state.update_data(screenshots=file_ids)
    await state.set_state(ExtraWorkFlow.waiting_description)
    await messages[0].answer("Опишите одним словом вашу проделанную работу за сегодня:", reply_markup=cancel_kb())
