...
from config import Role, PINGER_TELEGRAM_ID
...
async def cmd_setpassword(message: Message, command: CommandObject):
    ...
    login_url = f"{webapp_url}/login" if webapp_url else "(адрес сайта ещё не настроен — попроси создателя выполнить /addhome)"
...
async def cmd_info(message: Message):
    from utils.runtime_settings import get_info_text
