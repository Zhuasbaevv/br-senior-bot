...
from utils.access import get_user, set_user, all_users, role_name, can_manage, manageable_users, max_grantable_role, visible_orgs
...
def _candidates_for(admin) -> list:
    # manageable_users уже возвращает строго нижестоящих в своей организации/направлении
    # (или без ограничений для руководства+) — см. utils.access.can_manage.
    return manageable_users(admin)
...
async def grant_pick_role(callback: CallbackQuery, state: FSMContext):
    ...
    # Клавиатура roles_kb() и так не рисует кнопку для роли выше положенного, но
    # callback_data — обычная строка, и её можно отправить боту напрямую в обход
    # кнопок. Поэтому право выдавать именно эту роль перепроверяем и здесь.
    if role > max_grantable_role(admin):
        await callback.answer("❌ Вы не вправе выдавать эту роль.", show_alert=True)
        return
...
async def _finalize_grant(callback: CallbackQuery, state: FSMContext, bot: Bot, org_or_group: str):
    ...
    # Финальная перепроверка иерархии перед самой записью — все три условия
    # (сама цель управляема, сама роль не выше положенного, орг/группа в зоне
    # ответственности) когда-то проверялись только выбором кнопок, а не здесь.
    if not can_manage(admin, target) or role > max_grantable_role(admin):
        await callback.answer("❌ Недостаточно прав для этого действия.", show_alert=True)
        return
    # Роль SENIOR_WATCHER (с выбором ГРУППЫ, а не конкретной организации) вправе
    # выдавать только руководство+ (это уже гарантирует max_grantable_role выше),
    # так что здесь остаётся перепроверить только путь "выбор конкретной организации".
    if admin.role < Role.LEADERSHIP and org_or_group not in visible_orgs(admin):
        await callback.answer("❌ Эта организация вне вашей зоны ответственности.", show_alert=True)
        return
    ...
    # Объявление в VK-инфо-беседах (см. services/vk_bridge.py) — только для
    # Лидера/Следящего/Старшего следящего/Руководства, по ТЗ; свежий VK ID
    # берём из таблицы, а не из кэша (в UserInfo его нет).
    target_row = await run(sheets.get_user_row, target.telegram_id)
