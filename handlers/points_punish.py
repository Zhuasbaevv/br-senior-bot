...
from utils.access import get_user, all_users, role_name, can_manage
...
async def _vk_id_of(sheets, target) -> int | None:
    row = await run(sheets.get_user_row, target.telegram_id)
    vk = row.get("VK") if row else None
    return int(vk) if vk and str(vk).isdigit() else None
...
async def points_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    vk_id = await _vk_id_of(sheets, target)
    await vk_bridge.announce_points(target.nickname, vk_id, target.org, amount, reason)
...
async def punish_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    if target:
        ...
        if action == "give":
            ...
            vk_id = await _vk_id_of(sheets, target)
            await vk_bridge.announce_punishment(target.nickname, vk_id, target.org, ptype, reason)
        else:
