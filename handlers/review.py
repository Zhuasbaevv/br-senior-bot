...
from utils.access import get_user, all_users, role_name, visible_orgs
...
async def cb_apps_approve(callback: CallbackQuery, bot: Bot):
    ...
    # Раньше заявку можно было одобрить/отклонить по ID напрямую, даже если она
    # относится к организации вне зоны ответственности этого админа (подставлял
    # чужой app_id в callback_data — заявки самих организаций в списке не показывались,
    # но принять решение по ним всё равно было можно).
    if not preview_target or preview_target.org not in visible_orgs(admin):
        await callback.answer("❌ Эта заявка вне вашей зоны ответственности.", show_alert=True)
        return
...
async def apps_reject_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    preview_target = next((u for u in all_users() if u.nickname == (app_preview or {}).get("NickName")), None)
    if not preview_target or preview_target.org not in visible_orgs(admin):
        await message.answer("❌ Эта заявка вне вашей зоны ответственности.", reply_markup=admin_panel_kb(admin.role))
        return
