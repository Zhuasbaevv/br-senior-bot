...
def _visible_targets(admin) -> list:
    """Кого admin вправе видеть/администрировать — строго нижестоящих в своей
    организации/направлении (см. utils.access.can_manage). Раньше здесь просто
    отдавались ВСЕ из своей организации/направления, включая равных и старших —
    из-за чего, например, Следящий мог попытаться снять/выдать роль другому
    Следящему своей же организации."""
...
async def remove_user_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    # Повторная проверка иерархии здесь (а не только при показе списка выше) —
    # список кнопок можно обойти, отправив боту сырой callback_data напрямую.
    if target and not can_manage(admin, target):
        await message.answer("❌ Недостаточно прав для действий над этим пользователем.", reply_markup=admin_panel_kb(admin.role))
        return
...
async def cb_view_user(callback: CallbackQuery, state: FSMContext):
    ...
    if not can_manage(admin, target):
        await callback.answer("❌ Недостаточно прав — этот человек равен вам или выше по роли, либо не в вашей зоне ответственности.", show_alert=True)
        return
