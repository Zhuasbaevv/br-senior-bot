...
from utils.runtime_settings import set_webapp_url, set_info_text
from handlers.common import _in_game
...
async def cmd_settings(message: Message, command: CommandObject):
    ...
    if not target:
        ...
        return
    if target.role >= user.role and user.role != Role.CREATOR:
        await message.answer("❌ Этот человек равен вам или выше по роли — редактировать нельзя.")
        return
...
async def settings_nick_start(callback: CallbackQuery, state: FSMContext):
    ...
    target = get_user(target_id)
    if not admin or admin.role < Role.LEADERSHIP or not target or (target.role >= admin.role and admin.role != Role.CREATOR):
...
async def cmd_setinfo(message: Message, command: CommandObject):
    ...
    if not command.args:
        ...
        return
    await set_info_text(command.args)
    await message.answer("✅ Текст команды /info обновлён.")


# ============================================================ /addhome (создатель)
@router.message(Command("addhome"))
async def cmd_addhome(message: Message, command: CommandObject):
    """Устанавливает ссылку на сайт для кнопки '🌐 Сайт' в главном меню — без
    необходимости лезть в переменные окружения Railway. Хранится персистентно
    (лист 'Настройки'), переживает перезапуск бота."""
    user = get_user(message.from_user.id)
    if user is None or user.role != Role.CREATOR:
        return
    if not command.args or not command.args.strip():
        await message.answer("Использование: /addhome https://адрес-сайта")
        return
    ...
    await message.answer(f"✅ Ссылка на сайт обновлена: {url}\n\nКнопка «🌐 Сайт» в меню теперь ведёт сюда.")
