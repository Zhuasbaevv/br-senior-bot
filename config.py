...
# --- Web Push (уведомления в браузере/PWA, см. services/push.py) ---
# Сгенерировать свою пару: см. webapp/README.md, раздел про Web Push.
VAPID_PUBLIC_KEY = os.getenv("VAPID_PUBLIC_KEY", "")
VAPID_PRIVATE_KEY = os.getenv("VAPID_PRIVATE_KEY", "")
VAPID_CLAIM_EMAIL = os.getenv("VAPID_CLAIM_EMAIL", "admin@example.com")

# --- Мост с VK-ботом модерации (vk_moderation_bot) ---
# Адрес его HTTP-сервера (см. bot_app/announce_server.py в том проекте), без
# слэша на конце. Пусто = мост выключен, объявления в VK просто не отправляются.
VK_BOT_API_URL = os.getenv("VK_BOT_API_URL", "").rstrip("/")
# ОДИНАКОВОЕ значение должно быть прописано и здесь, и в переменных VK-бота
# (SITE_BRIDGE_SECRET там же).
...
SHEET_SETTINGS = "Настройки"  # key-value для настроек, которые задаются командами (например /addhome)
SHEET_QUIZZES = "Тесты"
SHEET_QUIZ_RESULTS = "Тесты_Ответы"
