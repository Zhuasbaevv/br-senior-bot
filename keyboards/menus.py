...
from config import Role, ALL_ORGS, SENIOR_GROUPS, PUNISHMENT_TYPES, MSK_TZ
...
def main_menu_kb(role: Role) -> InlineKeyboardMarkup:
    ...
    if role != Role.CREATOR:
        # Все, кроме создателя, управляются только через сайт — бот для них
        # оставляет только ссылку на него (плюс сами команды типа /setpassword,
        # /id по-прежнему работают, просто без кнопок меню).
        if webapp_url:
            b.button(text="🌐 Сайт", url=f"{webapp_url}/login")
        return b.as_markup()
