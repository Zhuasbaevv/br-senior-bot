...
_SHEET_HEADERS = {
    ...
    SHEET_QUIZZES: [
        "ID", "CreatedBy", "CreatedByRole", "Title", "Scope", "QuestionsJSON", "CreatedAt",
    ],
    SHEET_QUIZ_RESULTS: [
        "QuizID", "TelegramID", "NickName", "Org", "AnswersJSON",
...
class SheetsService:
    ...
    def rows_missing_today_report(self) -> list[dict]:
        """Кто сегодня ещё не сдал отчёт (пустая ячейка в колонке сегодняшней даты
        на текущем листе недели) — для напоминания (см. main.py)."""
        ws = self.ws(self.current_sheet_title)
        today_str = dt.datetime.now(MSK_TZ).strftime("%d.%m.%Y")
        col = self._find_day_column(ws, today_str)
        if col is None:
            return []
        ...
        for row_idx, row in enumerate(records[1:], start=2):
            ...
            if not nickname:
                continue
            cell_val = row[col - 1].strip() if len(row) >= col else ""
            if not cell_val:
                missing.append({"NickName": nickname, "row": row_idx})
        return missing

    def dashboard_stats(self, visible_orgs: list[str] | None = None) -> dict:
        """Данные для дашборда (/admin/dashboard): баллы по дням текущей недели,
        топ-5 по баллам, баллы по организациям — только по организациям из
        visible_orgs (зона видимости смотрящего), либо по всем, если None."""
        ws = self.ws(self.current_sheet_title)
        values = ws.get_all_values()
        headers = values[0] if values else []

        day_cols = list(range(4, 11))  # D..J, 1-based
        day_labels = [headers[c - 1].strip() if c - 1 < len(headers) else "" for c in day_cols]
        daily_totals = [0.0] * len(day_cols)

        users_points = []  # (nickname, points)
        org_totals: dict[str, float] = {}

        for org, (start, end) in ORG_ROW_RANGES.items():
            if visible_orgs is not None and org not in visible_orgs:
                continue
            org_sum = 0.0
            for row_idx in range(start, end + 1):
                ...
                for i, c in enumerate(day_cols):
                    try:
                        v = float(row[c - 1]) if c - 1 < len(row) and row[c - 1].strip() else 0.0
                    except ValueError:
                        v = 0.0
                    daily_totals[i] += v
                try:
                    total = float(row[10]) if len(row) > 10 and row[10].strip() else 0.0  # столбец K
                except ValueError:
                    total = 0.0
                users_points.append((nickname, total))
                org_sum += total
            org_totals[org] = org_sum

        top_users = sorted(users_points, key=lambda x: x[1], reverse=True)[:5]
        return {
    ...
    def export_current_sheet_rows(self, visible_orgs: list[str] | None = None) -> tuple[list[str], list[list[str]]]:
        """Заголовки + строки текущего листа недели, отфильтрованные по
        organизациям (для экспорта в Excel — см. webapp/app.py)."""
        ws = self.ws(self.current_sheet_title)
        values = ws.get_all_values()
        headers = values[0] if values else []
        ...
        rows = []
        for org, (start, end) in ORG_ROW_RANGES.items():
            if org not in visible_orgs:
                continue
            for row_idx in range(start, end + 1):
                if row_idx - 1 < len(values):
                    rows.append(values[row_idx - 1])
        return headers, rows
    ...
    def get_day_cell(self, nickname: str, date_obj: dt.date) -> dict:
        """Читает СЫРУЮ ячейку дня (D-J) для человека за конкретную дату — отличает
        'неактив' (там стоит '-') от 'вообще не сдавал' (пусто) от 'сдал N баллов'
        (число). Не создаёт лист, если его для этой недели ещё нет — просто говорит,
        что данных нет, чтобы не плодить пустые листы просмотром истории."""
        target_date = dt.datetime.combine(date_obj, dt.time())
        week_title = self._week_title_for(target_date)
        if week_title not in self._existing_titles():
            return {"sheet_exists": False, "value": None}

        ws = self.ws(week_title)
        row_idx = self.find_nick_row(nickname, sheet_title=week_title)
        if not row_idx:
            return {"sheet_exists": True, "value": None, "row_found": False}

        col_idx = self._find_day_column(ws, date_obj.strftime("%d.%m.%Y"))
        if col_idx is None:
            return {"sheet_exists": True, "value": None, "row_found": True}

        value = ws.cell(row_idx, col_idx).value
        return {"sheet_exists": True, "value": value, "row_found": True}
    ...
    def list_norms(self) -> list[dict]:
        """Все настроенные нормативы — для отображения таблицей (сайт/бот)."""
        ws = self.ws(SHEET_NORMS)
        return ws.get_all_records()

    # ================= Настройки (key-value, например ссылка на сайт из /addhome) =================
    def get_setting(self, key: str) -> str | None:
        ws = self.ws(SHEET_SETTINGS)
        records = ws.get_all_records()
        for r in records:
    ...
    def set_setting(self, key: str, value: str) -> None:
        ...
        if row_idx is None:
            ...
            ws.update(f"A{row_idx}:B{row_idx}", [[key, value]])

    # ================= Тесты (обучающие/аттестационные, создаются на сайте) =================
    # Одна строка листа "Тесты" = один тест целиком: сами вопросы и организации-адресаты
    # хранятся JSON-строкой в одной ячейке (а не отдельным листом на каждый вопрос) —
    # для количества вопросов, которое реально печатают руками через веб-форму, это
    # значительно проще и без потери гибкости, чем городить второй лист с внешним ключом.
    def next_quiz_id(self) -> int:
        ws = self.ws(SHEET_QUIZZES)
        ids = ws.col_values(1)[1:]
        nums = [int(x) for x in ids if x.isdigit()]
        return (max(nums) + 1) if nums else 1

    def create_quiz(
        self, created_by_nick: str, created_by_role: str, title: str,
        orgs: list[str], questions: list[str],
    ) -> int:
        ws = self.ws(SHEET_QUIZZES)
        quiz_id = self.next_quiz_id()
        now = dt.datetime.now(MSK_TZ).strftime("%d.%m.%Y %H:%M:%S")
        ws.append_row([
            quiz_id, created_by_nick, created_by_role, title,
    ...
    def list_quizzes(self) -> list[dict]:
        return self.ws(SHEET_QUIZZES).get_all_records()

    def get_quiz(self, quiz_id: int) -> dict | None:
        for q in self.list_quizzes():
            if str(q.get("ID")) == str(quiz_id):
                return q
        return None

    @staticmethod
    def quiz_orgs(quiz: dict) -> list[str]:
        try:
            return json.loads(quiz.get("Scope") or "[]")
        except (TypeError, ValueError):
            return []

    @staticmethod
    def quiz_questions(quiz: dict) -> list[str]:
        try:
            return json.loads(quiz.get("QuestionsJSON") or "[]")
        except (TypeError, ValueError):
            return []

    def _find_quiz_result_row(self, quiz_id: int, telegram_id: int) -> int | None:
        records = self.ws(SHEET_QUIZ_RESULTS).get_all_records()
        for i, r in enumerate(records, start=2):
            if str(r.get("QuizID")) == str(quiz_id) and str(r.get("TelegramID")) == str(telegram_id):
                return i
        return None

    def get_quiz_result(self, quiz_id: int, telegram_id: int) -> dict | None:
        row_idx = self._find_quiz_result_row(quiz_id, telegram_id)
        if row_idx is None:
            return None
        ws = self.ws(SHEET_QUIZ_RESULTS)
        headers = ws.row_values(1)
        values = ws.row_values(row_idx)
        values += [""] * (len(headers) - len(values))
        return dict(zip(headers, values))
    ...
    @staticmethod
    def quiz_created_at(quiz: dict):
        raw = quiz.get("CreatedAt")
        if not raw:
            return None
        try:
            return dt.datetime.strptime(raw, "%d.%m.%Y %H:%M:%S").replace(tzinfo=MSK_TZ)
        except (TypeError, ValueError):
            return None

    @staticmethod
    def is_quiz_expired(quiz: dict) -> bool:
        """Тест "живёт" ровно 24 часа с момента создания (по ТЗ) - дальше он
        перестаёт быть обязательным к прохождению, и страница /quiz/<id> для
        НЕ прошедших его больше не открывается. Уже отправленные до этого
        ответы (SHEET_QUIZ_RESULTS) никуда не деваются - истечение касается
        только "надо ли ещё заставлять его пройти", а не истории результатов."""
        created = SheetsService.quiz_created_at(quiz)
        if created is None:
            return False  # не смогли разобрать дату - не блокируем тест из-за этого
        return dt.datetime.now(MSK_TZ) - created > dt.timedelta(hours=24)

    def pending_quiz_for(self, telegram_id: int, org: str | None) -> dict | None:
        """Первый (по возрастанию ID) активный тест, назначенный организации этого
        человека, который он ещё не прошёл полностью (см. submit_quiz — Status
        становится "completed" только на финальном "Готово"; сорванная попытка,
        см. log_quiz_violation, ничего не сохраняет, так что тест снова считается
        непройденным и гейт на сайте покажет его заново). Тесты старше 24 часов
        (см. is_quiz_expired) считаются "исчезнувшими" — гейт их больше не требует."""
        if not org:
            return None
        for q in sorted(self.list_quizzes(), key=lambda r: int(r.get("ID") or 0)):
            if org not in self.quiz_orgs(q):
                continue
            if self.is_quiz_expired(q):
                continue
            existing = self.get_quiz_result(int(q["ID"]), telegram_id)
            if existing and existing.get("Status") == "completed":
                continue
            return q
        return None

    def submit_quiz(self, quiz_id: int, telegram_id: int, nickname: str, org: str, answers: list[str]) -> None:
        ws = self.ws(SHEET_QUIZ_RESULTS)
        row_idx = self._find_quiz_result_row(quiz_id, telegram_id)
        now = dt.datetime.now(MSK_TZ).strftime("%d.%m.%Y %H:%M:%S")
        row = [
            quiz_id, telegram_id, nickname, org,
            json.dumps(answers, ensure_ascii=False), "completed", now, now,
        ]
        if row_idx:
            ws.update(f"A{row_idx}:H{row_idx}", [row])
        else:
    ...
    def get_results_for_quiz(self, quiz_id: int) -> list[dict]:
        records = self.ws(SHEET_QUIZ_RESULTS).get_all_records()
        return [
            r for r in records
            if str(r.get("QuizID")) == str(quiz_id) and r.get("Status") == "completed"
        ]

    async def log_quiz_violation(self, quiz_id: int, nickname: str, org: str) -> None:
        """Только уведомление в канал логов — саму сорванную попытку не сохраняем
        (пустые ответы никому не нужны), тест просто остаётся непройденным."""
        now = dt.datetime.now(MSK_TZ).strftime("%d.%m.%Y %H:%M:%S")
        text = (
            f"🚨 <b>НАРУШЕНИЕ ПРИ ПРОХОЖДЕНИИ ТЕСТА</b>\n"
            f"👤 <b>Кто:</b> {nickname} ({org or '—'})\n"
            f"📄 <b>Тест:</b> #{quiz_id}\n"
            f"⚠️ Свернул вкладку/переключился на другую во время прохождения — "
            f"попытка не засчитана, тест нужно пройти заново.\n"
            f"⏱ <b>Время:</b> {now}"
        )
        await self._send_log_msg(text)
